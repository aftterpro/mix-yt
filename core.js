...
class UnifiedCore {
    ...
    setupEventListeners() {
        ...
        // Cola de reproducción - CORREGIDO PARA POPUP
        this.setupQueue();
        
        // Cerrar menús al hacer clic fuera
        document.addEventListener('click', (event) => {
            if (!event.target.closest('.delete-menu') && !event.target.closest('.queue-popup')) {
    ...
    // CORREGIDO: Configurar cola como popup
    setupQueue() {
    ...
    refreshLibraryView() {
        ...
        // CORREGIDO: Verificar si hay token válido y recargar playlists automáticamente
        if (window.isAuthorized && window.loadUserPlaylists) {
            console.log('🔄 Recargando playlists de biblioteca...');
            setTimeout(() => {
    ...
    createPlaylistCard(playlist) {
        ...
        // CORREGIDO: Mostrar botón eliminar para playlists de YouTube también
        const canDelete = playlist.source !== YOUTUBE_LIBRARY_SOURCE_ID;
    ...
    async createPlaylistPopup(playlist) {
        if (playlist.source === YOUTUBE_LIBRARY_SOURCE_ID && !playlist.isLoaded && playlist.videos.length === 0) {
            ...
            playlist = playlistsData.find(p => p.id === playlist.id);
        }
                ...
                <div class="playlist-popup-content">
                    ${playlist.videos.length === 0 ? 
                        ...
                        playlist.videos.map((video, index) => {
                            // CORREGIDO: Mostrar duración correctamente
                            const duration = video.duration && video.duration > 0 
                                ? this.formatDuration(video.duration) 
                                : '--:--';
    ...
    // ELIMINADA: updateQueueDisplay - Ya no se usa, reemplazada por popup
    ...
    clearQueue() {
        if (confirm('¿Estás seguro de que quieres borrar toda la cola?')) {
            ...
            this.closeQueuePopup(); // Cerrar popup después de limpiar
    ...
    // CORREGIDO: Arreglar reproducción en crossfade
    async playNextVideo() {
        ...
        // Actualizar estado ANTES de cualquier operación de reproductor
        currentPlayingInfo = {
        ...
        try {
            ...
            // CORREGIDO: Cargar y configurar el siguiente reproductor correctamente
            await new Promise((resolve, reject) => {
                ...
                // CORREGIDO: Configurar volumen a 0 ANTES de reproducir
            ...
            // CORREGIDO: Asegurar que el video se reproduce ANTES del crossfade
            await nextPlayerInstance.playVideo();
            
            // Esperar un momento para asegurar que está reproduciendo
    ...
    // CORREGIDO: Mejorar crossfade para asegurar reproducción
    startCrossfade(prevPlayer, nextPlayer) {
        ...
        // CORREGIDO: Crossfade de audio mejorado
        const duration = CROSSFADE_DURATION * 1000;
        const steps = 60;
        const stepTime = duration / steps;
        let step = 0;
        
        // ASEGURAR que el video nuevo esté reproduciéndose
        setTimeout(() => {
            try {
                if (nextPlayer.getPlayerState() !== YT.PlayerState.PLAYING) {
                    console.log('🔄 Forzando reproducción del nuevo video...');
                    nextPlayer.playVideo();
                }
            } catch (e) {
                console.warn('Error forzando reproducción:', e);
            }
        }, 100);
        ...
        crossfadeInterval = setInterval(() => {
            ...
            try {
                ...
                // ASEGURAR que el nuevo video siga reproduciéndose
                if (nextPlayer.getPlayerState() !== YT.PlayerState.PLAYING) {
                    nextPlayer.playVideo();
                }
            } catch (e) {
            ...
            if (step >= steps) {
                ...
                setTimeout(() => {
                    try {
                        ...
                        // VERIFICAR FINAL que el nuevo video esté reproduciéndose
                        if (nextPlayer.getPlayerState() !== YT.PlayerState.PLAYING) {
                            console.log('🔄 Verificación final: iniciando reproducción...');
                            nextPlayer.playVideo();
                        }
                    } catch (e) {
    ...
    createSearchResultCard(video, videoId) {
        ...
        // CORREGIDO: Manejar duración correctamente
