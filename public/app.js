...
class SpotifyColorEngine {
    ...
    applyPalette(palette) {
        ...
        const rgb = rgbMatch ? rgbMatch[0] : '29, 185, 84';
...
class NowPlayingManager {
    ...
    update(video) {
        ...
        if (title) {
            ...
            title.classList.toggle('scrolling-text', video.title && video.title.length > 30);
        }
...
function activateTab(tabName) {
    ...
    setupControls() {
        document.getElementById('np-play-pause-btn')?.addEventListener('click', () => {
            ...
            if (!reproduccionIniciada) {
                ...
                const tryPlay = () => {
                    if (playersInitialized) playFirstVideo();
            ...
            } catch (e) { console.warn('Error en play-pause:', e); }
        });
    ...
    setupProgressBarScrubbing() {
        ...
        bar.addEventListener('mousedown', (e) => { isDragging = true; this.seekTo(e, bar); });
        document.addEventListener('mousemove', (e) => { if (isDragging) this.seekTo(e, bar); });
        document.addEventListener('mouseup', () => { isDragging = false; });
        bar.addEventListener('touchstart', (e) => { isDragging = true; this.seekTo(e.touches[0], bar); });
        document.addEventListener('touchmove', (e) => { if (isDragging) this.seekTo(e.touches[0], bar); });
    ...
    setupVolumeScrubbing() {
        ...
        const setVolume = (e) => {
            ...
            document.getElementById('np-volume-fill').style.width = vol + '%';
            const player = currentPlayer === 1 ? player1 : player2;
    ...
    startProgressUpdater() {
        setInterval(() => {
            ...
            try {
                ...
                const state = player.getPlayerState();
                const icon = document.getElementById('np-play-icon');
                if (icon) icon.className = state === YT.PlayerState.PLAYING ? 'fas fa-pause' : 'fas fa-play';
            } catch (e) {}
        }, 500);
    }
}

window.nowPlayingManager = null;
...
function displaySearchResultsPiped(results = []) {
    ...
    results.forEach((video, index) => {
        ...
        `;
        el.querySelector('.result-add-btn').addEventListener('click', (e) => { e.stopPropagation(); addToPlaylist(normalizedVideo); });
        el.querySelector('.result-play-now-btn').addEventListener('click', (e) => { e.stopPropagation(); insertAndPlayNow(normalizedVideo); });
...
function onPlayerStateChange(event) {
    if (event.data === YT.PlayerState.PLAYING) {
        ...
        if (window.nowPlayingManager) window.nowPlayingManager.update(fullVideo);
        setTimeout(() => {
...
function updatePlaylistDOM() {
    ...
    window.playlistVideos.forEach((video, index) => {
        ...
        item.innerHTML = `
            ...
            <div class="queue-thumb-wrap">
                <img src="${video.thumbnail || `https://i.ytimg.com/vi/${video.videoId}/mqdefault.jpg`}" alt="${video.title}"
    ...
    if (currentItem) setTimeout(() => currentItem.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 100);

    const playBtn = document.getElementById('np-play-pause-btn');
    if (playBtn) {
        playBtn.style.opacity = window.playlistVideos.length > 0 ? '1' : '0.4';
...
const addToPlaylist = (videoData) => {
    if (!videoData?.videoId) { mostrarMensajeFlotante('⚠️ Error: datos inválidos'); return; }
    const videoObject = {
        videoId: videoData.videoId,
        title: videoData.title || 'Sin título',
        thumbnail: videoData.thumbnail || `https://i.ytimg.com/vi/${videoData.videoId}/mqdefault.jpg`,
    ...
    if (window.playlistVideos.some(v => v.videoId === videoObject.videoId)) {
...
function initializePlayers() {
    ...
    function checkBothReady() {
        if (p1Ready && p2Ready) {
            ...
            playerReady = true;
            console.log('✅ Ambos players inicializados');
        }
    }
...
function onPlayerError(event) {
    const errores = { 2: 'ID inválido.', 5: 'Restricción.', 100: 'No encontrado.', 101: 'No permitido.', 150: 'No permitido.' };
    const msg = errores[event.data] || 'Error del reproductor';
    console.warn(`⚠️ Error YT [${event.data}]: ${msg}`);
    mostrarMensajeFlotante(`⚠️ ${msg} - Siguiente...`);
...
// =============================================
// FIX PANTALLA NEGRA: transición sin ocultar antes de tiempo
// =============================================
function _doTransition(nextPlayerNum, nextPlayerObj, nextVideoId, onDone) {
    const currentEl = document.getElementById(`player${currentPlayer}`);
    const nextEl = document.getElementById(`player${nextPlayerNum}`);

    // Cargar video en el player oculto
    nextPlayerObj.loadVideoById(nextVideoId);

    // Hacer visible el siguiente player (opacidad 0 primero)
    if (nextEl) {
        nextEl.style.visibility = 'visible';
        nextEl.style.pointerEvents = 'none';
        nextEl.style.opacity = '0';
        nextEl.style.transition = 'opacity 1.2s ease';
    }

    // Esperar a que el nuevo player comience a reproducir antes de hacer fade
    ...
    const startFade = () => {
        ...
        requestAnimationFrame(() => {
            ...
            if (currentEl) {
                currentEl.style.transition = 'opacity 1.2s ease';
    ...
    // Escuchar cuando el nuevo player empieza a reproducir
    const checkPlaying = setInterval(() => {
        try {
            if (nextPlayerObj.getPlayerState() === YT.PlayerState.PLAYING ||
                nextPlayerObj.getPlayerState() === YT.PlayerState.BUFFERING) {
    ...
    // Fallback: iniciar fade aunque sea después de 1.5s (evita espera infinita)
...
function playNextVideo() {
    ...
    _doTransition(nextPlayerNum, nextPlayerObj, nextVideoId, () => crossfadeAudio());
}
...
window.playSpecificVideoWithCrossfade = function(targetIndex) {
    ...
    _doTransition(nextPlayerNum, nextPlayerObj, video.videoId, () => crossfadeAudio());
};

function crossfadeAudio() {
    // currentPlayer ya fue actualizado a nextPlayerNum por _doTransition
    const prevPlayerNum = currentPlayer === 1 ? 2 : 1;
    const prevPlayer = prevPlayerNum === 1 ? player1 : player2;
    ...
    const interval = setInterval(() => {
        progress += increment;
        prevPlayer?.setVolume?.(Math.max(0, Math.cos(progress * 0.5 * Math.PI) * 100));
        nextPlayer?.setVolume?.(Math.min(100, Math.sin(progress * 0.5 * Math.PI) * 100));

        if (progress >= 1) {
            clearInterval(interval);
            prevPlayer?.pauseVideo?.();
            prevPlayer?.setVolume?.(100); // Reset para próxima vez
...
function startMonitoring() {
    if (!monitorInterval) monitorInterval = setInterval(monitorPlayers, 500);
}
...
function monitorPlayers() {
    ...
    if (!_triggerCache[videoId]) {
        _triggerCache[videoId] = window.sponsorBlockManager
            ? window.sponsorBlockManager.calculateCrossfadeTriggerTime(duration, videoId, CROSSFADE_DURATION)
            : duration - CROSSFADE_DURATION;
        console.log(`📍 Trigger ${videoId}: ${_triggerCache[videoId].toFixed(1)}s`);
    }
    ...
    if (flooredTime % 30 === 0 && flooredTime !== window._lastLogTime) {
        ...
        console.log(`⏱️ P${currentPlayer} | ${flooredTime}s/${duration.toFixed(0)}s | Trigger@${triggerTime.toFixed(0)}s`);
    }
...
function extractPlaylistId(url) {
    ...
    } catch (e) { return url.length > 10 ? url.trim() : null; }
}
...
function displayPlaylist(playlist) {
    if (!playlist?.items?.length) { mostrarMensajeFlotante('No se encontraron videos'); return; }
    ...
    mostrarMensajeFlotante(`✅ Añadidas ${loaded.length} canciones`);
    updatePlaylistDOM();
}
...
function playFirstVideo() {
    ...
    if (p2) { p2.style.opacity = '0'; p2.style.visibility = 'hidden'; p2.style.pointerEvents = 'none'; }
...
window.addEventListener('error', (e) => {
    if (e.message?.includes('youtube.com') || e.message?.includes('ERR_BLOCKED_BY_CLIENT')) e.preventDefault();
});
...
const performSearch = async (query) => {
    ...
    try {
        ...
        if (window.isAuthorized && window.gapi?.client?.youtube) {
            const response = await gapi.client.youtube.search.list({ part: 'snippet', q, maxResults: 25, type: 'video' });
...
function setupEventListeners() {
    ...
    document.getElementById('añadirUrlButton')?.addEventListener('click', async () => {
        ...
        if (!playlistId) { mostrarMensajeFlotante('⚠️ URL no válida'); return; }
...
class LyricsManager {
    ...
    async loadLyricsForCurrentVideo(video) {
        ...
        if (!this.cache.has(cacheKey)) {
            container.innerHTML = `<div class="lyrics-loading"><div class="lyrics-spinner"></div><p>Buscando letra...</p></div>`;
        }
        ...
        try {
            ...
            container.innerHTML = `<div class="empty-state"><i class="fas fa-music" style="opacity:0.3"></i><p>Letra no disponible</p><small>${clean.title}</small></div>`;
        }
    }
    ...
    prefetchNext() {
        ...
        if (!this.cache.has(key)) this.getOrFetchLyrics(clean, key).catch(() => {});
    }
    ...
    renderLyricsUI(data, artist, title) {
        ...
        container.innerHTML = `
            <div class="lyrics-header">
                ...
                <button id="btn-translate-lyrics" class="np-ctrl-btn lyrics-translate-btn" title="Traducir al español">
    ...
    async toggleTranslation(data, isSynced) {
    ...
    parseLRC(lrc) {
        ...
        return lrc.split('\n').map(line => {
            ...
            if (!m) return null;
            return { time: parseInt(m[1])*60 + parseInt(m[2]) + parseFloat('0.'+m[3]), text: m[4].trim() };
        }).filter(Boolean);
    }
...
class RelatedManager {
    ...
    async loadRelatedForVideo(video) {
        ...
        container.innerHTML = `<div class="related-loading"><div class="related-spinner"></div><p>Buscando recomendaciones...</p></div>`;

        try {
            const playlistVideo = window.playlistVideos?.find(v => v.videoId === currentVideoId);
            let artist = (playlistVideo?.artist || video.author || video.artist || '')
                .replace(/VEVO$/i, '').replace(/\s*-\s*Topic$/i, '').replace(/Official/i, '').trim();
            let title = (playlistVideo?.title || video.title || '')
                .replace(/[\(\[].*?[\)\]]/g, '')
                .replace(/official|video|audio|lyric|hd|hq|mv|4k|en vivo|live/gi, '').trim();
            artist = artist.split(/\s(\(|\[)?(ft\.|feat\.|,|&)/i)[0].trim();

            const queries = [];
            if (artist && title) { queries.push(`${artist} ${title}`); queries.push(`${artist} mix`); }
            else if (title) queries.push(title);
            queries.push('top hits music 2024');

            let items = [];

            // Priorizar GAPI si está autenticado
            if (window.isAuthorized && window.gapi?.client?.youtube) {
                for (const q of queries) {
                    try {
                        const response = await gapi.client.youtube.search.list({
                            part: 'snippet', q, maxResults: 15, type: 'video'
                        });
                        const candidates = (response.result.items || [])
                            .filter(v => v.id.videoId !== currentVideoId)
                            .map(v => ({
                                videoId: v.id.videoId,
                                title: v.snippet.title,
                                thumbnail: v.snippet.thumbnails.medium?.url,
                                artist: v.snippet.channelTitle.replace(/VEVO$/i, '').replace(/\s*-\s*Topic$/i, ''),
                                duration: 0
                            }));
                        if (candidates.length >= 3) { items = candidates.slice(0, 12); break; }
                    } catch (e) { console.warn('GAPI related error:', q, e); }
                }
            }
            ...
            if (items.length === 0) {
                for (const q of queries) {
                    try {
                        const data = await window.youtubeJSClient.search(q);
                        const candidates = (data?.items || []).filter(v => v.videoId !== currentVideoId);
                        if (candidates.length >= 3) { items = candidates.slice(0, 12); break; }
                    } catch (e) { console.warn('Proxy related error:', q, e); }
                }
            }
            ...
            items.forEach(v => {
                ...
                item.innerHTML = `
                    ...
                    <div class="related-info">
                        ...
                        <div class="related-artist">${v.artist || v.uploaderName || ''}</div>
                    </div>
                    <button class="related-add-btn" title="Añadir a cola"><i class="fas fa-plus"></i></button>
                `;
                item.querySelector('.related-add-btn').addEventListener('click', (e) => { e.stopPropagation(); addToPlaylist(v); });
                item.addEventListener('click', (e) => { if (!e.target.closest('.related-add-btn')) insertAndPlayNow(v); });
                list.appendChild(item);
            });
            container.appendChild(list);

        } catch (e) {
            container.innerHTML = `<div class="empty-state"><i class="fas fa-magic-wand-sparkles" style="opacity:0.3"></i><p>Sin recomendaciones disponibles</p></div>`;
        }
    }
}
